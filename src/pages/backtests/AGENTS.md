...
## OVERVIEW
`src/pages/backtests/` contains the routed backtest list, configuration, and detail pages. These routes launch historical simulations, poll running jobs, and render result and report follow-up UI.

## WHERE TO LOOK
| Task | Location | Notes |
|---|---|---|
| Inventory management | `list.tsx` | newest-first list, running progress bars, terminal delete, completed summary text |
| Simulation configuration | `config.tsx` | existing/new portfolio mode, optional default template creation, benchmark and LLM settings |
...
## CONVENTIONS
- `detail.tsx` relies on `useBacktest()` polling every 5 seconds for `PENDING` and `RUNNING` rows instead of manual timers in the page.
...
- `detail.tsx` shows progress and recent activity while active, then switches to charts and trade history once `results` are available.
