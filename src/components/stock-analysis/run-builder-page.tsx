...
import { useCreateConversation, useStockAnalysisConversations, useStockAnalysisSettings } from "@/hooks/use-stock-analysis";
...
export function RunBuilderPage() {
  ...
  const conversationsQuery = useStockAnalysisConversations(portfolioId || undefined, { includeArchived: false });
          ...
          <CardContent className="flex flex-col gap-4 py-12 sm:flex-row sm:items-center sm:justify-between">
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                {portfoliosQuery.error instanceof Error ? portfoliosQuery.error.message : "Check the backend connection and try again."}
              </p>
            </div>
    ...
    <div className="max-w-6xl space-y-6 p-6">
      <section className="rounded-2xl border border-border/60 bg-gradient-to-br from-sky-50 via-background to-amber-50 p-6 shadow-sm dark:from-sky-950/20 dark:via-background dark:to-amber-950/10">
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                Pick a portfolio, open a conversation thread, then launch a real backend run.
              </p>
            </div>
          </div>
          <div className="w-full max-w-sm space-y-2">
            <span className="text-xs font-medium uppercase tracking-[0.2em] text-muted-foreground">Portfolio</span>
        ...
        {selectedPortfolio ? (
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-muted-foreground">
            <Badge variant="outline" className="gap-1.5"><FolderKanban className="size-3" />{selectedPortfolio.positionCount} positions</Badge>
            <Badge variant="outline">{selectedPortfolio.balanceCount} balances</Badge>
            <Badge variant="outline">Base currency: {selectedPortfolio.baseCurrency}</Badge>
            {settingsQuery.isPending ? <Badge variant="outline" className="gap-1.5"><Loader2 className="size-3 animate-spin" /> Loading defaults</Badge> : null}
            {settingsQuery.data?.defaultLlmConfigId ? <Badge variant="outline">Default config ready</Badge> : null}
            {settingsQuery.data?.defaultPromptTemplateId ? <Badge variant="outline">Default template ready</Badge> : null}
          </div>
        ) : null}
      </section>
      ...
      {settingsQuery.isError ? (
        <Alert>
          ...
          <AlertDescription>{settingsQuery.error instanceof Error ? settingsQuery.error.message : "Run defaults could not be loaded, but you can still configure a run manually."}</AlertDescription>
        </Alert>
      ) : null}
      ...
      {selectedPortfolio && !analysisEnabled ? (
        <Alert>
          ...
          <AlertDescription>Conversation browsing stays available, but run execution remains blocked until the portfolio setting is enabled.</AlertDescription>
        </Alert>
      ) : null}
            ...
            <>
              <Card className="border-border/60 bg-muted/10">
                <CardContent className="flex flex-col gap-4 pt-6 sm:flex-row sm:items-center sm:justify-between">
                  <div className="space-y-1">
                    <p className="text-sm font-medium">{selectedConversation.title?.trim() || `${selectedConversation.symbol} coverage`}</p>
                    <p className="text-sm text-muted-foreground">Build the next run for {selectedConversation.symbol} inside this conversation.</p>
                  </div>
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <Badge variant="secondary">{selectedConversation.symbol}</Badge>
                    <Badge variant="outline">{selectedConversation.runCount} runs</Badge>
                    <Badge variant="outline">{selectedConversation.versionCount} versions</Badge>
                  </div>
                </CardContent>
              </Card>

              <RunBuilderForm conversationId={selectedConversation.id} isAnalysisEnabled={analysisEnabled} onRunStarted={setActiveRunId} portfolioId={portfolioId} settings={settingsQuery.data} symbol={selectedConversation.symbol} />
            </>
          ) : (
            <Card className="border-border/60 bg-muted/10">
              <CardContent className="py-12 text-center text-sm text-muted-foreground">Choose an existing conversation or create one on the left to unlock the run builder.</CardContent>
            </Card>
          )}
